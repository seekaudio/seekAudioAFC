...
public class PeerConnectionClient {
  ...
  public void createPeerConnection(final VideoSink localRender, final List<VideoSink> remoteSinks,
    ...
    // 正确设置isInitiator标志
    this.isInitiator = signalingParameters.initiator;
    Log.d(TAG, "[WebRTC]创建PeerConnection，角色: " + (isInitiator ? "发起方" : "应答方"));
...
/*    final AudioDeviceModule adm = createJavaAudioDevice();*/
    ...
    // 旧版本适配（如果 AAR 是 M95-）
    try {
      Class<?> envClass = Class.forName("org.webrtc.Environment");
      Constructor<?> constructor = envClass.getConstructor();
      Object environment = constructor.newInstance();
      Method builderMethod = PeerConnectionFactory.class.getMethod("builder", envClass);
      builder = (PeerConnectionFactory.Builder) builderMethod.invoke(null, environment);
    } catch (Exception e) {
      // 降级到新版本 API（如果反射失败）
  ...
  private void createPeerConnectionInternal() throws Exception {
    if (factory == null || isError) {
      throw new Exception("[WebRTC]Peerconnection factory is not created");
    }
    Log.d(TAG, "[WebRTC]Create peer connection.");
    ...
    rtcConfig.iceTransportsType = PeerConnection.IceTransportsType.ALL; // 允许所有传输类型

    peerConnection = factory.createPeerConnection(rtcConfig, pcObserver);
    Log.d(TAG, "[WebRTC]createPeerConnection done: " + peerConnection);
    ...
    // isInitiator已经在createPeerConnection方法中设置，这里不需要重新设置
  ...
  void createOffer() {
    ...
    peerConnection.createOffer(new SdpObserver() {
      @Override public void onCreateSuccess(SessionDescription sdp) {
        Log.d(TAG,"[WebRTC] Offer创建成功");
        peerConnection.setLocalDescription(this, sdp);
        assert DirectRTCClient.tcpClient != null;
        DirectRTCClient.tcpClient.sendSignalingMessage("offer", sdp.description);
        Log.d(TAG,"[WebRTC]发送offer sdp: [" + sdp.description + "]");
      }
      @Override public void onSetSuccess() { Log.d(TAG,"[WebRTC] setLocalDescription成功"); }
      @Override public void onCreateFailure(String error) { Log.d(TAG,"[WebRTC] Offer创建失败: " + error); }
  ...
  public void addRemoteIceCandidate(final IceCandidate candidate) {
    executor.execute(() -> {
      if (peerConnection != null && !isError) {
        Log.d(TAG, "[WebRTC]" + (isInitiator ? "发起方" : "应答方") + " 添加远程ICE候选: " + candidate);
        if (queuedRemoteCandidates != null) {
          ...
          peerConnection.addIceCandidate(candidate, new AddIceObserver() {
            ...
            public void onAddSuccess() {
              Log.d(TAG, "[WebRTC]ICE候选添加成功: " + candidate);
            }
            @Override
            public void onAddFailure(String error) {
              Log.e(TAG, "[WebRTC]ICE候选添加失败: " + error);
            }
          });
        }
      }
  ...
  public void setRemoteDescription(SdpObserver sdpObserver, final SessionDescription desc) {
    executor.execute(() -> {
      if (peerConnection == null || isError) {
        Log.e(TAG, "[WebRTC]无法设置远程SDP：PeerConnection为空或已出错");
        return;
      }
      ...
      Log.d(TAG, "[WebRTC]" + (isInitiator ? "发起方" : "应答方") + " 设置远程SDP: " + desc.type);
  ...
  /**
   * 检查PeerConnection是否准备好接受SDP
   */
  public boolean isReadyForSDP() {
  ...
  public PeerConnection.SignalingState getSignalingState() {
    return peerConnection != null ? peerConnection.signalingState() : null;
  }
  ...
  private AudioTrack createAudioTrack() {
    Log.d(TAG, "[WebRTC]创建音频轨道，enableAudio: " + enableAudio);
    audioSource = factory.createAudioSource(audioConstraints);
    localAudioTrack = factory.createAudioTrack(AUDIO_TRACK_ID, audioSource);
    localAudioTrack.setEnabled(enableAudio);
    Log.d(TAG, "[WebRTC]音频轨道创建完成，ID: " + AUDIO_TRACK_ID + ", 启用状态: " + enableAudio);
    return localAudioTrack;
  }

  @Nullable
  private VideoTrack createVideoTrack(VideoCapturer capturer) {
    Log.d(TAG, "[WebRTC]创建视频轨道，renderVideo: " + renderVideo);
    ...
    Log.d(TAG, "[WebRTC]视频轨道创建完成，ID: " + VIDEO_TRACK_ID + ", 启用状态: " + renderVideo);
  ...
  private void drainCandidates() {
    if (queuedRemoteCandidates != null) {
      Log.d(TAG, "[WebRTC]处理队列中的远程ICE候选者，数量: " + queuedRemoteCandidates.size());
      for (IceCandidate candidate : queuedRemoteCandidates) {
        peerConnection.addIceCandidate(candidate, new AddIceObserver() {
          @Override
          public void onAddSuccess() {
            Log.d(TAG, "[WebRTC]队列中的候选者添加成功: " + candidate);
          }
          @Override
          public void onAddFailure(String error) {
            Log.e(TAG, "[WebRTC]队列中的候选者添加失败: " + error);
          }
        });
      }
  ...
  private class PCObserver implements PeerConnection.Observer {
    ...
    public void onIceConnectionChange(final PeerConnection.IceConnectionState newState) {
      executor.execute(() -> {
        Log.d(TAG, "[WebRTC]IceConnectionState: " + newState);
        if (newState == IceConnectionState.CONNECTED) {
          Log.d(TAG, "[WebRTC]ICE连接成功建立");
          events.onIceConnected();
        } else if (newState == IceConnectionState.DISCONNECTED) {
          Log.d(TAG, "[WebRTC]ICE连接断开");
          events.onIceDisconnected();
        } else if (newState == IceConnectionState.FAILED) {
          Log.e(TAG, "[WebRTC]ICE连接失败");
          reportError("ICE connection failed.");
        } else if (newState == IceConnectionState.CHECKING) {
          Log.d(TAG, "[WebRTC]ICE连接检查中...");
        } else if (newState == IceConnectionState.NEW) {
          Log.d(TAG, "[WebRTC]ICE连接新建");
        }
      });
    }
    ...
    public void onAddStream(final MediaStream stream) {
      Log.d(TAG, "[WebRTC]收到远程媒体流，视频轨道: " + stream.videoTracks.size() +
              ", 音频轨道: " + stream.audioTracks.size());
  ...
  private class SDPObserver implements SdpObserver {
    ...
    public void onCreateSuccess(final SessionDescription desc) {
      ...
      Log.d(TAG, "[WebRTC]设置localDescription为: " + desc.type);
      
      executor.execute(() -> {
        if (peerConnection != null && !isError) {
          Log.d(TAG, "[WebRTC]Set local SDP from " + desc.type);
          peerConnection.setLocalDescription(this, newDesc);
          Log.d(TAG, "[WebRTC]" + (isInitiator ? "发起方" : "应答方") + " setLocalDescription完成");
        } else {
          Log.e(TAG, "[WebRTC]无法设置本地SDP：PeerConnection为空或已出错");
        }
      });

      events.onLocalDescription(newDesc);
    }
